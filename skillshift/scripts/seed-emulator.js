async function seed() {
  ...
  const projectId = 'skillshift';
  // pequena espera para garantir que o emulador subiu
  ...
  for (const u of users) {
    ...
    if (!data.localId) { console.error('Erro ao criar usuário', data); continue; }
    const setClaims = await fetch(`http://localhost:9099/emulator/v1/projects/${projectId}/accounts:update`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
